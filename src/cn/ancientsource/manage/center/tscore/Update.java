...
public class Update extends JFrame {
	...
	private static final long serialVersionUID = -6585881719980360121L;
		...
		public Update(String account) {
			...
			try {
				ResultSet rs = Jdbc.SqlStatement("SELECT * FROM `学生成绩` WHERE `学号`=" + account);
				rs.next();
		...
		public void upDate() {
			try {
				...
				if(!JT_S1.getText().equals("") && !JT_S1.getText().equals("请输入成绩1")){
					...
					Jdbc.SqlStatementUpdate("UPDATE `学生成绩` SET `成绩1`=" + JT_S1.getText() + " WHERE `学号`=" + JT_SID.getText());
				}else{
					Jdbc.SqlStatementUpdate("UPDATE `学生成绩` SET `成绩1`=0 WHERE `学号`=" + JT_SID.getText());
				}
				if(!JT_S2.getText().equals("") && !JT_S2.getText().equals("请输入成绩2")){
					sum += Integer.valueOf(JT_S2.getText());
					Jdbc.SqlStatementUpdate("UPDATE `学生成绩` SET `成绩2`=" + JT_S2.getText() + " WHERE `学号`=" + JT_SID.getText());
				}else{
					Jdbc.SqlStatementUpdate("UPDATE `学生成绩` SET `成绩2`=0 WHERE `学号`=" + JT_SID.getText());
				}
				if(!JT_S3.getText().equals("") && !JT_S3.getText().equals("请输入成绩3")){
					sum += Integer.valueOf(JT_S3.getText());
					Jdbc.SqlStatementUpdate("UPDATE `学生成绩` SET `成绩3`=" + JT_S3.getText() + " WHERE `学号`=" + JT_SID.getText());
				}else{
					Jdbc.SqlStatementUpdate("UPDATE `学生成绩` SET `成绩3`=0 WHERE `学号`=" + JT_SID.getText());
				}
				Jdbc.SqlStatementUpdate("UPDATE `学生成绩` SET `总分`=" + sum + " WHERE `学号`=" + JT_SID.getText());
