...
public class Update extends JFrame {
	 ...
	 * @author luo
	 */
	private static final long serialVersionUID = 1611261548893596402L;
	...
	public Update(String Account) {
		...
		try {
			ResultSet rs = Jdbc.SqlStatement("SELECT * FROM `学生信息` WHERE `学号`=" + Account);
			rs.next();
	...
	private void PanelSet() {
		...
		base = new JPanel() {
			 ...
			 * @author luo
			 */
			private static final long serialVersionUID = -7599312259034624538L;
	...
	public void upDate() {
		if (JT_SID.getText().equals("") || JT_SID.getText().equals("请输入学号") || JT_PWD.getText().equals("")
				|| JT_PWD.getText().equals("请输入密码")) {
		...
		try {
			...
			if (!JT_Name.getText().equals("") && !JT_Name.getText().equals("请输入姓名")) {
				Jdbc.SqlStatementUpdate("UPDATE `学生信息` SET `姓名`='" + JT_Name.getText() + "' WHERE `学号`=" + acc);
			}
			Jdbc.SqlStatementUpdate("UPDATE `学生信息` SET `学号`='" + JT_SID.getText() + "',`性别`='"
					+ JCB_Sex.getSelectedItem() + "',`院系`='" + JCB_Faculty.getSelectedItem() + "',`专业`='"
					+ JCB_Speciality.getSelectedItem() + "',`登入密码`='" + JT_PWD.getText() + "' WHERE `学号`=" + acc);
